...
/**
 * @param {Object} lastProcessedPosition Last processed position from ZK
 * @param {Array} blocksList List of block intervals
 * @returns {number} Index of the blocks interval to start from
 */
function initBlocksListPosition(lastProcessedPosition, blocksList) {
  if (lastProcessedPosition && lastProcessedPosition.blockNumber) {
    const lastProcessedBlock = lastProcessedPosition.blockNumber;
    for (let i = 0; i < blocksList.length; i++) {
      if (blocksList[i][0] <= lastProcessedBlock && blocksList[i][1] >= lastProcessedBlock) {
        return i;
      }
    }
    throw new Error(`Block number ${lastProcessedBlock} not found in blocks list`);
  }
  return 0;
}
