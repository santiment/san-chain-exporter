...
async function getPastEventsExactContracts(web3, fromBlock, toBlock) {
  ...
  for (let contractsIndex = 0; contractsIndex < exactContractsArray.length; ++contractsIndex) {
    const contractAddresses = exactContractsArray[contractsIndex];
    const replaceAddress = contractAddresses[contractAddresses.length - 1];

    for (let oldContractAddressIndex = 0; oldContractAddressIndex < contractAddresses.length - 1; ++oldContractAddressIndex) {
      let events = getPastEvents(web3, fromBlock, toBlock, contractAddresses[oldContractAddressIndex]);
      const decodedEvents = await decodeEvents(web3, events)
      const filteredEvents = filterEvents(decodedEvents);
      const result = fixContractAddresses(filteredEvents);
...
function fillExactContractsArray() {
  exactContractsArray = fs.readFileSync("contract_mapping.csv", {encoding: "utf8"})
      .split("\n")
      .filter((line) => line != 0)
...
function changeContractAddress(events, oldContract, newContract) {
  for(let event of events) {
    if (oldContract == event.contract) {
      event.contract = newCont
    }
  }
}
