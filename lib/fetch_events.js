...
// Map of changed contracts: { new contract address: old address }
const changed_contracts = {
  "0xc011a73ee8576fb46f5e1c5751ca3b9fe0af2a6f": "0xc011a72400e58ecd99ee497cf89e3775d4bd732f", // SNX
  "0x57ab1ec28d129707052df4df418d58a2d46d5f51": "0x57ab1e02fee23774580c119740129eac7081e9d3" // sUSD
}
...
async function getPastEvents(web3, fromBlock, toBlock) {
  let events = await getRawEvents(web3, fromBlock, toBlock)
...
/*
  Some tokens may migrate to new proxy contracts, in order to correctly aggregate transactions from new contracts
  we replace new contract addresses by legacy ones.

  There are at least 2 places which sensitive to changing of contract address:
  1. CH-table daily metrics jobs which aggregate transactions and match them to asset medata using contract address.
  2. Flink job etherbi-flink which calculates eth_stacks and eth_balances.
 */
function fixContractAddresses(events) {
  for(let event of events) {
    const old_contract_address = changed_contracts[event.contract]
    if (old_contract_address != undefined) {
      event.contract = old_contract_address
    }
  }
}
