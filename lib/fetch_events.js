...
// An array of arrays. Each element is an erray in the format ['old_address1', 'old_address2', ... , 'replace_address']
let exactContractsArray = []

async function getPastEventsExactContracts(web3, fromBlock, toBlock) {
  if (0 == exactContractsArray.length) {
    fillExactContractsArray();
  }

  let resultsAggregation = [];

  for (const contractAddresses of exactContractsArray) {
    const oldContractAddresses = contractAddresses.slice(0, contractAddresses.length - 1);
    const newContract = contractAddresses[contractAddresses.length - 1];

    console.log("Getting events for contracts ", oldContractAddresses)
    let events = await getPastEvents(web3, fromBlock, toBlock, oldContractAddresses);
    const result = changeContractAddresses(events, oldContractAddresses, newContract);
    resultsAggregation.concat(result);
  }
...
async function getPastEvents(web3, fromBlock, toBlock, contractAddress) {
  ...
  addCustomTokenDistribution(result, fromBlock, toBlock)

  return result
}

function fillExactContractsArray() {
  exactContractsArray = fs.readFileSync("./lib/contract_mapping.csv", {encoding: "utf8"})
      .split("\n")
      .filter((line) => line != 0)
      .map((line) => line.split(",").map((element) => element.trim()))

  logger.info(`Running in 'exact contracts mode', ${exactContractsArray.length} contracts will be monitored.`)
}
...
function changeContractAddresses(events, oldContracts, newContract) {
  for(let event of events) {
    if (oldContracts.includes(event.contract)) {
      event.contract = newContract;
    }
  }
}
