...
async function decodeTransferEvent(web3, event, blockTimestamps) {
  ...
  let to = decodeAddress(event["topics"][2]).toLowerCase()
  if (to == QNT_contract && event["address"].toLowerCase() == QNT_contract) {
...
async function decodeWETHDepositEvent(web3, event, blockTimestamps) {
  if (event["address"].toLowerCase() != "0xc02aaa39b223fe8d0a0e5c4f27ead9083c756cc2"
      || event["topics"].length != 2) {
    return null
  }
