...
class ContractEditor {
  ...
  async getPastEventsExactContracts(pastEventsFunction, fromBlock, toBlock) {
    let resultsAggregation = [];

    for (const contractOverwrite of this.contractsOverwriteArray) {
      const events = await pastEventsFunction(fromBlock, toBlock, contractOverwrite.oldAddresses);
...
function getPastEventsExactContractsFunction(web3) {
  const pastEventsFunction = getPastEventsFunction(web3);
  return async function inner_function (fromBlock, toBlock) {
    return await contractEditor.getPastEventsExactContracts(pastEventsFunction, fromBlock, toBlock);
  }
}
...
module.exports = {
  getPastEventsExactContractsFunction,
